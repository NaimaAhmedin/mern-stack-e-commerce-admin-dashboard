...
const Profile = () => {
  ...
  const handleInputChange = (e) => {
    ...
    // Handle nested sellerDetails field
  ...
  const handleSave = async () => {
    try {
      ...
      Object.keys(profile).forEach(key => {
        if (key !== 'profileImage' && key !== 'role' && key !== 'sellerDetails') {
          formData.append(key, profile[key]);
        }
      });

      // Append business license number
      if (profile.sellerDetails?.businessLicense) {
        formData.append('businessLicenseNumber', profile.sellerDetails.businessLicense);
      }
  ...
  const profileFields = [
    ...
    { key: 'businessLicense', label: 'Business License Number', type: 'text', nestedKey: 'sellerDetails.businessLicense' },
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-100 flex items-center justify-center p-6">
      <div className="w-full max-w-xl bg-white rounded-2xl shadow-2xl overflow-hidden transform transition-all duration-300 hover:scale-105">
        <div className="bg-gradient-to-r from-indigo-500 to-blue-500 p-6 text-center">
          <h2 className="text-3xl font-bold text-white tracking-wide">Seller Profile</h2>
        </div>
            ...
            {profileFields.map((field, index) => (
              <div key={index} className="relative">
                ...
                <div className="relative">
                  <input 
                    type={field.type}
                    name={field.key === 'businessLicense' ? 'businessLicense' : field.key}
                    value={
                      field.key === 'businessLicense' 
                        ? profile.sellerDetails?.businessLicense || '' 
                        : profile[field.key] || ''
                    }
